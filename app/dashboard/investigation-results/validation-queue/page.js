...
export default function InvestigationValidationQueuePage() {
  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/investigation-results/validation-queue?${params}`);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Investigation Results Validation Queue</h1>
        <p className="text-gray-600 mt-2">Review and validate completed investigation reports</p>
      </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {results.map((result) => (
                    <tr key={result.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm">
                          <div className="font-medium text-gray-900">{result.investigation.name}</div>
                          <div className="text-gray-500">{result.investigation.code}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {getCategoryBadge(result.investigation.category)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm">
                          <div className="text-gray-900">{result.reportedBy}</div>
                          <div className="text-gray-500 text-xs">{formatDate(result.reportedDate)}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex flex-col gap-1">
                          <Link
                            href={`/dashboard/investigation-results/enter/${result.requestId}/${result.testRequestItemId}`}
                            className="text-blue-600 hover:text-blue-800 font-medium"
                          >
                            {result.status === 'validated' ? 'View' : 'Review & Validate'}
                          </Link>
                          <Link
                            href={`/dashboard/investigation-results/print/${result.id}`}
                            target="_blank"
